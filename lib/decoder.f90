...
!$ use omp_lib
  ...
  type, extends(ft2h_decoder) :: counting_ft2h_decoder
     integer :: decoded
  end type counting_ft2h_decoder
  ...
  type(counting_ft2h_decoder) :: my_ft2h
...
1004             format(a12,1x,a4,i5,i6,i7,i3)
  ...
  if(params%nmode.eq.25) then
     call timer('decft2h ',0)
     call my_ft2h%decode(ft2h_decoded,id2,params%nQSOProgress,params%nfqso,    &
          params%nfa,params%nfb,params%ndepth,                               &
          logical(params%lapcqonly),ncontest,mycall,hiscall)
     call timer('decft2h ',1)
     go to 800
  endif
...
10  call flush(6)
  ...
  subroutine ft2h_decoded (this,sync,snr,dt,freq,decoded,nap,qual)
    use ft2h_decode
    implicit none
    ...
    write(*,2001) params%nutc,snr,dt,nint(freq),decoded0,annot
2001 format(i6.6,i4,f5.1,i5,' ~ ',1x,a37,1x,a2)

    if(ios13.eq.0) then
       write(13,2002,err=20) params%nutc,nint(sync),snr,dt,freq,0,decoded0
2002   format(i6.6,i4,i5,f6.1,f8.0,i4,3x,a37,' FT2H')
       flush(13)
    endif
...
20  call flush(6)
    ...
    select type(this)
    type is (counting_ft2h_decoder)
       this%decoded = this%decoded + 1
    end select

    return
  end subroutine ft2h_decoded
